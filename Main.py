...
# --- CONFIGURATION ---
NUM_CLICK_POSITIONS = 15
CONFIG_DIR = "multiclicker_configs"
ICON_FILE = "my_icon.ico"
AHK_SCRIPT_NAME = "click_backend.ahk"

# --- Set AutoHotkey Path ---
# Using the specific path you provided for the AutoHotkey v2 executable.
AHK_EXECUTABLE_PATH = r"C:\Program Files\AutoHotkey\v2\AutoHotkey64.exe"
AHK_SCRIPT_PATH = os.path.join(os.path.dirname(__file__), AHK_SCRIPT_NAME)


# --- Core Clicker Thread (MODIFIED FOR AHK) ---
...
class App(customtkinter.CTk):
    ...
    def _create_top_frame(self):
        ...
        customtkinter.CTkLabel(top_frame, text="Clicker Mode:").grid(row=0, column=0, padx=10, pady=10)
        customtkinter.CTkRadioButton(top_frame, text="Single", variable=self.click_mode_var, value="Single", command=self.on_mode_change, state="disabled").grid(row=0, column=1, padx=10, pady=10, sticky="w")
        customtkinter.CTkRadioButton(top_frame, text="Multi", variable=self.click_mode_var, value="Multi", command=self.on_mode_change).grid(row=0, column=2, padx=10, pady=10, sticky="w")
    ...
    def _create_config_frame(self):
        ...
        config_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=10)
        config_frame.grid_columnconfigure((0, 1, 2), weight=1)
        customtkinter.CTkLabel(config_frame, text="Configuration Profile:").grid(row=0, column=0, columnspan=3, padx=5, pady=(5,0), sticky="w")
        self.config_combobox = customtkinter.CTkComboBox(config_frame, values=[])
        self.config_combobox.grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky="ew")
        customtkinter.CTkButton(config_frame, text="Load", command=self._load_config).grid(row=2, column=0, padx=5, pady=5, sticky="ew")
        customtkinter.CTkButton(config_frame, text="Save", command=self._save_config).grid(row=2, column=1, padx=5, pady=5, sticky="ew")
        customtkinter.CTkButton(config_frame, text="Delete", command=self._delete_config, fg_color="firebrick").grid(row=2, column=2, padx=5, pady=5, sticky="ew")
    ...
    def _build_multi_clicker_ui(self):
        ...
        customtkinter.CTkLabel(config_frame, text="Delay Between Clicks (s):").pack(side='left', padx=5, pady=5)
        ...
        customtkinter.CTkCheckBox(config_frame, text="Loop Sequence", variable=self.loop_var).pack(side='left', padx=10, pady=5)
        points_frame = customtkinter.CTkScrollableFrame(self.main_frame, label_text="Click Sequence")
        points_frame.pack(fill='both', expand=True, padx=5, pady=5)
        headers = ["#", "On", "Set Position", "Coordinates", "Clicks"]
        points_frame.grid_columnconfigure(3, weight=1)
        for col, header in enumerate(headers): customtkinter.CTkLabel(points_frame, text=header, font=customtkinter.CTkFont(weight="bold")).grid(row=0, column=col, padx=5, pady=5)
        self.points_ui = []
        for i in range(NUM_CLICK_POSITIONS):
            row_num = i + 1
            on_var = customtkinter.IntVar(value=1 if i < 3 else 0)
            pos_label = customtkinter.CTkLabel(points_frame, text="Not Set", anchor="w")
            clicks_entry = customtkinter.CTkEntry(points_frame, width=50)
            clicks_entry.insert(0, "1")
            customtkinter.CTkLabel(points_frame, text=f"#{row_num}").grid(row=row_num, column=0, padx=5)
            customtkinter.CTkCheckBox(points_frame, text="", variable=on_var).grid(row=row_num, column=1, padx=5)
            import pyautogui 
            customtkinter.CTkButton(points_frame, text="Get Pos", width=80, command=lambda r=i, pg=pyautogui: self.get_mouse_position(r, pg)).grid(row=row_num, column=2, padx=5, pady=2)
            pos_label.grid(row=row_num, column=3, padx=10, sticky="ew")
            clicks_entry.grid(row=row_num, column=4, padx=5)
            self.points_ui.append({'on_var': on_var, 'pos_label': pos_label, 'clicks_entry': clicks_entry, 'coords': None})
    ...
    def _save_config(self):
        ...
        name = name.strip()
        config_data = {"mode": "Multi", "delay": self.multi_delay_entry.get(), "loop": self.loop_var.get(), "points": []}
        for point_ui in self.points_ui: config_data["points"].append({"enabled": point_ui["on_var"].get(), "coords": point_ui["coords"], "clicks": point_ui["clicks_entry"].get()})
        try:
            with open(os.path.join(CONFIG_DIR, f"{name}.json"), 'w') as f: json.dump(config_data, f, indent=4)
            self._refresh_config_list()
